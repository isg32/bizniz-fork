...
async def oauth2_initiate(
    ...
    """
    Initiates the OAuth2 login flow for a server-side redirect.

    This endpoint gets the auth details from PocketBase, stores the
    necessary secrets (state and code_verifier) in secure HttpOnly cookies,
    and returns the auth_url for the frontend to redirect the user to.
    """
    ...
    # These values are generated by the PocketBase SDK service
    # and are needed for the callback
    ...
    if not all([state, code_verifier, auth_url]):
        raise HTTPException(
            ...
            detail="OAuth2 provider is not configured correctly in PocketBase admin.",
        )

    # Store the state and verifier in cookies.
    # The callback will read these to verify the request.
    cookie_age_seconds = 60 * 10  # 10 minutes
...
@router.get(
    "/oauth2/{provider}/callback",  # <-- Changed from POST to GET
...
async def oauth2_callback(
    ...
    response: Response,
    provider: str,
    # These now come from query parameters, not a JSON body
    code: str,
    state: str,
    # These come from the cookies we set in the /oauth2/{provider} endpoint
    pb_state: Annotated[Optional[str], Cookie(alias="pb_oauth_state")] = None,
    pb_verifier: Annotated[Optional[str], Cookie(alias="pb_oauth_verifier")] = None,
):
    """
    The final step of the OAuth2 flow (server-side).

    Google redirects the user's browser here (a GET request).
    This endpoint validates the state, retrieves the code_verifier from
    the cookie, and exchanges the code for a user session and JWT.
    """
    if not pb_state or not pb_verifier:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Login session expired or cookies are missing. Please try logging in again.",
        )

    if state != pb_state:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="OAuth state mismatch. Possible CSRF attack. Please try logging in again.",
        )

    # This is the same redirect_uri that was used to generate the auth_url
    # It MUST match what is in your PocketBase Admin UI and Google Cloud Console
    ...
    response.delete_cookie("pb_oauth_state")
    response.delete_cookie("pb_oauth_verifier")
